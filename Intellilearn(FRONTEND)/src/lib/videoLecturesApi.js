...
export function getVideoLectures(page = 1, limit = 100) {
  return apiFetch(`/api/video-lectures/?page=${page}&limit=${limit}`, { method: 'GET', cacheTTL: 45000 });
}

export function createVideoLecture(payload) {
  return apiFetch('/api/video-lectures/', {
    method: 'POST',
    body: JSON.stringify(payload),
  }).then((data) => {
    invalidateApiCacheByPath('/api/video-lectures');
    return data;
  });
}
